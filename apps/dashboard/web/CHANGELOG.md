...
- ba6aeb1: The Audit page now reads as an activity log: each entry is a sentence naming who acted, grouped by day, with a search box and a filter for people versus the platform's own jobs and integrations. Clicking an entry opens it in place with the action in words, the app it touched and a before-and-after of what changed. Recent activity on the Overview now opens the exact entry it shows.
...
- b780f67: The dashboard's Overview is now a home page of its own instead of a second copy of the Apps list. It opens with one plain sentence about how your apps and integrations are doing, saying whether a problem is on the app's side or the supplier's, followed by a list of everything that needs attention with a link to where to fix it. Below that are your app cards, each with its health in words and an Observe link, then your integrations and the latest entries from the audit log. If one of these can't be loaded, only that section says so and the rest of the page still works.
...
- b9e7ac5: The SQL console and the table browse now say that they read the app's data as stored, including personal data, and that every query is recorded in the access log. The dashboard docs state the same, with the Events explorer's two known limits.
