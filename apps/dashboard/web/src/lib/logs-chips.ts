 ...
 * name those filters; anything else is the message search in Lines and the event type in
 * Events. A malformed `level:` or `invocation:` is refused with the reason rather than
 * searched for as text: the reader typed a filter, and matching it as a message would
 * answer a different question with an empty list.
 */
export function parseBarText(text: string, mode: LogMode): { add: Partial<ObsQuery> } | { error: string } | null {
  ...
  if (m) {
    ...
    if (mode === 'events') return { error: `${key} filters the Lines mode.` };
    if (key === 'level') {
