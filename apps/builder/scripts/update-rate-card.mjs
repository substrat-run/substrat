#!/usr/bin/env node
/**
 * Rate-card snapshot generator — builder-harness.md §2.2, issue #663 row 1.
 *
 ...
 * (`pnpm --filter @substrat-run/builder update-rate-card`), never in CI and
...
/**
 * The models the studio serves and bills — the pair table (model-pairs.ts)
...
function reconcile(name, md, ll) {
	...
	if (md.tiers.length !== ll.tiers.length) {
		...
		// exact undercharge case (builder-harness.md §2.2).
		mismatches.push(
