...
| Module | Export | What it is |
|---|---|---|
| `platform-sweep.ts` | `runPlatformSweep`, `startPlatformSweeper`, `EventSink`, `AccessLogSink`, `PlatformSweepOptions`, `PlatformSweepReport` | One pass of every scheduled thing the platform does, in a fixed order — migration reconciliation first, so a scope that failed to migrate is skipped by every phase after it, and the access-log drain last. Between them: drain retryable effects and pending platform intents, re-run the provision hook where it never ran (#1172), reconcile connectors, reap expired snapshots, run each vertical's declared schedules (#383), judge each module's declared [`freshness`](/concepts/modules) expectations (#1272), reap long-archived scopes and lapsed tenants, and drain each scope's domain events to Tier 2 through the **`EventSink`** (#1413) before the staff access log goes through the `AccessLogSink` and what shipped is pruned. The two sinks are injected, and absent a sink nothing drains — a self-host with none configured is a supported posture, it just keeps everything. Every unit's outcome goes to `recordSweepRun` as a `_substrat_sweep_runs` row (#1255), and the pass returns a `PlatformSweepReport` per phase. It holds **no timer**: a node deployment calls `startPlatformSweeper`, a Cloudflare one arms a sweeper DO's alarm |
