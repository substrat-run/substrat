...
| `timeline.ts` | `readTimeline`, `readHistory`, `TimelineReader` | The sanctioned **read** of `_substrat_outbox` for one entity: `readTimeline` is the envelope walk (id, type, when, who), `readHistory` adds the fat payload, the K-34 authorization, the K-42 impersonation stamp, the emitting `operation` (the `invoke()` string; null for a consumer's emit or a pre-column row) and the `version` the emitting code was deployed as (read from the outbox column, never the envelope; null when no version identity was present) — null payload after a shred is a supported answer. Both build a `SELECT` and nothing else, and neither checks permission: the caller's `assertAllowed` on the entity comes first, on the line above. Paged like an HTTP list (`ListPage` in, `Page` out) |
