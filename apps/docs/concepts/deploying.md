...
- **A push repairs its own installs.** Each scope records the version its provision hook last
  ran against (`provisionedVersionId`), and the platform sweep re-runs `/internal/reconcile` on
  every active install whose receipt is behind the version it now serves — so whatever a new
  release's `onProvision` mints reaches existing installs too, which is why that hook must be
  idempotent. The how-to is in [Deploying a vertical](/guide/deploying#ship-it-substrat-push).
