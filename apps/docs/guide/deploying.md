...
the PR does) **rebinds the new version onto the same fork**. When routing moves between distinct
version deployments on a host with a per-version resolver, it copies the fork's data before binding;
unchanged routing and co-located incoming versions need no copy. Successive pushes roll migrations
forward on the same data. A write made during a copy may be lost
