...
site's own `llms-full.txt`. Ask it something the documentation answers and it cites the
...
- The desk sees your message and nothing about you. Without a signed identity from the
  embedding site, a visitor is anonymous, and a conversation lives in your browser's
  local storage only.
- Replies arrive by polling, so an answer can take a few seconds to land.
- Cost is not hidden from the people running the desk. Every assistant turn records
  what it consumed, which is part of the point of the demo.
