...
> feature. Companion to [api-surface.md](api-surface.md) (the OpenAPI
> emit this lands in) and [kernel-design.md](kernel-design.md).

## 0. The gap this closed

Substrat had no error model. It had seven copies of one, none of them typed.
...
- plus shop, rally, todo, handlebar — **and the scaffold template**
  ...
  which is what made this a replicator rather than a fixed-size cleanup.

The control plane did the same thing at greater scale and had already written its own
...
1. **The API surface documented no failures.** [`openapi.ts:57-62`](../../packages/contracts/src/openapi.ts#L57-L62)
   emitted `400/401/403/404` with a prose description and **no schema**. Every operation
   documented its success shape and was silent about every way it can fail — precisely the
...
old one died. **v0.1 proposed a sentinel prefix on `message`, and then guessed that
