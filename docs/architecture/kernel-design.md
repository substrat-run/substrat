...
interface ScopeHost {
  ...
  // Act AS a principal on a staff actor's behalf (K-42) — the same gates as getScope,
  // plus a second actor every record keeps. Read-only unless `writes: true`, time-boxed
  // and checked per invoke, and audited BEFORE the stub exists.
  getImpersonatedScope(
    request: ImpersonationRequest, tenantId: TenantId, scopeId: ScopeId,
...
interface OperationContext {
  ...
  impersonation: Impersonation | null; // K-42: who is REALLY acting, or null (the norm)
}
```
...
interface DomainEvent<T = unknown> {
  ...
  impersonation?: Impersonation;  // K-42: the staff actor really acting, when this write
                                  //   happened under a support session. Absent = nobody
                                  //   was, and `actor` is then the whole truth.
...
- `authorization` is stamped by the kernel from the checks the operation actually
  ...
  that is kept.
- `impersonation` is stamped the same way and for the same argument (K-42). An
  invocation through `getImpersonatedScope` carries **two** actors: `actor` stays the
  principal the operation acted AS — which is what makes a handler impersonatable with
  no change to it — and this column names the staff actor who was really at the
  keyboard, the reason they gave, and the window it was valid in. A session swap would
  have recorded only the first, which is precisely the version that fails an audit.
  Read-only sessions (the default) emit nothing at all, so a row carrying this column is
  always a write somebody explicitly opted into.
