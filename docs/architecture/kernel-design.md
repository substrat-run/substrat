...
interface DomainEvent<T = unknown> {
  ...
  impersonation?: {               // K-42: WHO was really acting, when `actor` was being
    by: PrincipalId               //   acted AS. Kernel-stamped like `authorization`;
      | { staff: PlatformActorId };//  absent means nobody was impersonating, which is the
    reason: string;               //   ordinary case rather than an unrecorded one.
    expiresAt: Instant;
  };
...
- `authorization` is stamped by the kernel from the checks the operation actually
  ...
  that is kept.
- `impersonation` is stamped the same way and for the same reason (K-42). `actor`
  stays the principal whose authority the write ran under — which is also who the
  permission model answered as — and this names the person behind them. A support
  session's writes are therefore attributable to BOTH, in the row itself rather
  than through a session table nobody joins; the same field rides the denial log
  and the platform-intent journal, so every record a scope keeps about who did what
  answers the question the same way. It is bounded (`expiresAt`, capped at
  `IMPERSONATION_MAX_MINUTES` and enforced per invoke) and reason-carrying by
  schema, and `getImpersonatedScope` is the only door that can set it.
