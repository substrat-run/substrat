...
status: proposed
layer: plan
description: Audit of docs/ and the restructure plan. This document.
---

# Proposal: restructuring `docs/` — an audit first, a folder tree second

**Status: draft for review. Not approved, not merged.** Written after reading all 55
markdown files in `docs/`, both decision logs end to end, and verifying every status claim
against the repository. The folder tree is the smallest part of this document and the last
section, deliberately: **the corpus's problem is not that the files are in the wrong
directories — it is that a third of them assert things about the system that are no longer
true, and nothing in the repo can notice.**
...
The consequence is not cosmetic. [dashboard.md](../design/dashboard.md) is the
...
- [permission-registry-enforcement.md](../design/permission-registry-enforcement.md) is
  ...
  **zero hits** in `master-plan.md`. So a shipped decision holds a number belonging to a
  ...
  either document.
- [builder-studio.md](../design/builder-studio.md) §13 proposes **D-47 through D-55** —
...
- [engine-protocol.md](../design/engine-protocol.md) records that kernel open question 11
...
[proposals/hosting-and-certification.md](hosting-and-certification.md) opens: "**Status:
...
- `master-plan.md` D-43 links `docs/design/preview-and-snapshots.md` — a repo-root path
...
My first pass suggested deduplicating `docs/design/control-plane.md` against
...
**Phase 3 — the move.**
Directory restructure plus a link-fixing pass across the ~30 referencing files outside
`docs/`. Mechanical once Phase 1 has classified everything.
...
[why-substrat.md](../why-substrat.md) states it contains material a public page must not —
