...
structure) and decision 32 ([hosting-and-certification](./hosting-and-certification.md))
...
named builders through human admission ([self-serve-deploy](../architecture/self-serve-deploy.md) §7). Tighten
...
| EU/US jurisdiction **enforcement** (residency guarantee) | B — Regional Services is a **CF Enterprise add-on** | [control-plane](../architecture/control-plane.md) §; jurisdiction default `global` |
...
Phasing follows the deploy trust models ([self-serve-deploy](../architecture/self-serve-deploy.md) §3): our
