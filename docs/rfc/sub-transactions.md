...
> [kernel-design.md](../architecture/kernel-design.md) (the scope-host contract),
> [platform-neutral-surface.md](../architecture/platform-neutral-surface.md) (the non-Cloudflare mapping)
> and [platform-intents.md](../architecture/platform-intents.md) (one of the four things that must roll back).
>
...
>
> **v0.2** rewrites the adapter contract (§3) after taking a third adapter seriously. v0.1
...
So *"catch an engine error and keep going"* has **three different meanings** across the
substrates we claim portability for:

| Substrate | What a caught engine error does today |
|---|---|
...
contract suite pins it — a Postgres adapter could pass today's suite and still differ here.
...
- [ ] it all commits **once**
- [ ] the **original error** reaches the caller's `catch`, unwrapped
...
- [ ] **the caller's own error is recoverable too** — catching a non-engine error raised inside
      an atomic leaves the transaction usable (§1: the assertion a Postgres host must satisfy,
      and the one today's suite cannot express)
