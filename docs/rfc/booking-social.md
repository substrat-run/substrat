...
> adapter/connector triage, D-23 tuple engine, D-30 outbox), [kernel-design.md](../architecture/kernel-design.md)
> (§3.2 directory, §4.3 the three audiences, DO = consistency domain),
> [control-plane.md](../architecture/control-plane.md) (§6 identity seam, `_substrat_outbox`).
...
[engine-booking.md §0](../engines/booking.md).
...
inventory *tenants*, [kernel-design.md §3.2](../architecture/kernel-design.md)) and burden each with
...
- **One global authhero pool for players** — one identity across all clubs. This is
  ...
  distinct from the per-tenant white-label **staff** pools ([kernel-design.md §4.3](../architecture/kernel-design.md)).
...
the sanctioned way effects leave a scope ([control-plane.md](../architecture/control-plane.md), D-30).
