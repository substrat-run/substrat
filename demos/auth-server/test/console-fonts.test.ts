...
/** Every `.css` under `app/src`, since the ban is on the bundle, not on one file. */
function stylesheets(dir: string): string[] {
  const out: string[] = [];
  for (const name of readdirSync(dir, { withFileTypes: true })) {
    const full = join(dir, name.name);
    if (name.isDirectory()) out.push(...stylesheets(full));
...
const sheets = stylesheets(appSrc).map((path) => ({
  path: relative(appSrc, path),
...
describe('the console webfonts are self-hosted', () => {
  it('no stylesheet reaches a font CDN, directly or through @substrat-run/ui', () => {
    for (const { path, css } of sheets) {
      ...
      expect(code(css), `${path} must not fetch a font over a CDN`).not.toMatch(
        /fonts\.(googleapis|gstatic)\.com/,
      );
      expect(code(css), `${path} must import @substrat-run/ui tokens individually, not the barrel`).not.toMatch(
  ...
  it('ships the latin upright subset of each face and nothing else', () => {
    const fonts = sheets.find((s) => s.path.endsWith('fonts.css'));
    expect(fonts, 'app/src/console/fonts.css is where the faces are declared').toBeDefined();

    const faces = rules(fonts!.css).filter((r) => r.selector === '@font-face');
  ...
  it('leaves the themed hand-off vocabulary on the system stack', () => {
    ...
    // else's brand.
    expect(tokens!.css).not.toMatch(/Geist/);
  });
});
