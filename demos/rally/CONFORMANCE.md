...
**6 pairs driven** across 6 of this package's 7 narrowed checks.

| Operation | Permission | Narrows to | Driven |
| --- | --- | --- | --- |
| `rally/add-player` | `booking:create` | `reservation`, id from `reservationId` | `reservation` |
| `rally/cancel-subscription` | `booking:read` | `member`, resolved in the handler | — see §3 — |
| `rally/confirm-booking` | `booking:confirm` | `reservation`, id from `reservationId` | `reservation` |
| `rally/open-up` | `booking:confirm` | `reservation`, id from `reservationId` | `reservation` |
| `rally/played-with` | `booking:read` | `member`, id from `memberId` | `member` |
| `rally/timeline` | `booking:read` | `member`, id from `entityId` | `member` |
| `rally/wallet` | `booking:read` | `member`, id from `memberId` | `member` |
...
| --- | --- |
| `rally/cancel-subscription` | declares 'resolved' (the member is read off the subscription row) — the entity id is not in the input, so the harness cannot reach the entity |
