...
- **Callout, Meridian, Manyfold, Todo, ticket0, shop and handlebar have no dev auth branch.** Their `… dev` scripts
...
- **Callout, Meridian, Manyfold, Todo, ticket0, shop and handlebar are OIDC-only** (`docs/architecture/oidc-only-demos.md`):
  ...
  exercising real accounts; **rally is the last demo still running Better Auth**, and the
  last still carrying `ALLOW_DEV_HEADER` (see oidc-only-demos.md for why). Shop is the one OIDC-only demo
