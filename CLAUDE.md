...
- **Every demo vertical is OIDC-only** (`docs/architecture/oidc-only-demos.md`):
  ...
  exercising real accounts, and is now the ONLY workspace member that runs Better Auth at
  all. No vertical carries `ALLOW_DEV_HEADER` or an `x-principal` seam any more. Shop keeps
  an **anonymous** browse principal, which is not a credential store: a storefront must
  answer "what may someone who has not signed in see". Shop is the one OIDC-only demo
...
`*.generated.*` names its producer. Mark 2 is held in full on every file that declares:
the header reads `GENERATED by <tool> from <source>`, and "by" alone is refused, because
the source is the file a reader needs to edit instead. A `*.generated.json` cannot carry
the header, so it is allowlisted in the tool beside its producer (`COMMENTLESS`) and
refused otherwise. The files allowed to *contain* the header without being generated —
the emitters — are named one by one; a directory-wide exemption once covered an emitted
file with no suffix, the exact case #987 reported. Mark 3 is deliberately NOT inferred:
...
because another vertical's deployed code parses it. A base the checkout does not hold is exit 2,
never "every model.json is new". CI runs the tool's own tests first. What makes the schema BE in
`model.json`: `lint:permissions` refuses (exit 2) a vertical whose manifest exports a
...
in the tool that emits them — and `scripts/lake/outbox-stream-schema.generated.json`, which
does carry the suffix, names its producer in `lint:generated-marks`'s `COMMENTLESS` list
instead of in a header it cannot hold. Never invent a fourth category; an in-repo source with no gate
