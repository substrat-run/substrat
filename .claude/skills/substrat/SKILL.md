...
**Versioning is owned by changesets** — never hand-edit the version, never rely on push's
auto-bump (it advances the registry past `package.json` and they drift). Set it up when
the vertical first deploys:
...
```json
"changeset": "changeset",
"release": "pnpm test && pnpm typecheck && pnpm lint:boundaries && changeset version && substrat push --promote prod"
```
...
changesets, bumps `package.json`, writes `CHANGELOG.md`, and `substrat push --promote prod`
deploys **that exact version** and points prod at it. Changesets needs a git repo with a
commit on the base branch (`git init -b main`) — scaffolds that aren't repos yet must
init before the first release.
