...
export class SqliteScopeHost implements ScopeHost {
            ...
            const bound = this.directory
              .prepare(
                `SELECT scope_id, tenant_id, vertical_version_id FROM scopes
                 WHERE vertical = ? AND tenant_id = ? AND status = 'active' AND forked_from IS NULL AND kind != 'preview'`,
              )
  ...
  /**
   * #1572: `_substrat_sweep_runs_intent` with `kind` in it, on a directory whose index
   * predates that. `CREATE UNIQUE INDEX IF NOT EXISTS` matches on the name alone, so the
   * bootstrap cannot do this: on an existing directory it sees the old index and skips,
   * and the schedule/freshness collision the new key refuses stays live for good.
   *
   * Detected from `sqlite_master.sql`, like the table rebuilds above, and replaced as
   * DROP-then-CREATE in one transaction (`rebuildAtomically`). Less rides on that than on
   * a table rebuild — the index holds no rows, and one dropped but not re-created comes
   * back on the next open, because the bootstrap creates it by name — but there is then
   * no committed directory with no dedupe key at all. The CREATE cannot meet a row it
  ...
  private ensureSweepRunsIntentKind(): void {
    const row = this.directory
      ...
      .get('_substrat_sweep_runs_intent') as { sql: string } | undefined;
    if (row && sweepRunsIntentHasKind(row.sql)) return;
    this.rebuildAtomically(`
      DROP INDEX IF EXISTS _substrat_sweep_runs_intent;
      ${SWEEP_RUNS_INTENT_INDEX}
    `);
  }
