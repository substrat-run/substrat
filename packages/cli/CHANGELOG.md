...
- Updated dependencies [30b09c6]
  ...
  - @substrat-run/model-view@0.2.25
...
- 9ed127c: `substrat push` reads the declared surface again when the vertical is named by a relative directory (`substrat push demos/ticket0`). 0.35.0 wrote that path into an import specifier, where a relative path is a bare package name, so every such push failed with "Cannot find package".
...
- Updated dependencies [429cc84]
  ...
  - @substrat-run/model-view@0.2.19
...
- 7bfccc5: `substrat push` and `substrat push --check` now say what they are doing about the entity
...
- Updated dependencies [1a6fe4d]
  ...
  - @substrat-run/model-view@0.2.16
...
- Updated dependencies [f58aa74]
  ...
  - @substrat-run/model-view@0.2.15
...
- Updated dependencies [7e6f925]
  ...
  - @substrat-run/model-view@0.2.14
...
- Updated dependencies [221f94a]
  ...
  - @substrat-run/model-view@0.2.13
...
- Updated dependencies [1b2506c]
  ...
  - @substrat-run/model-view@0.2.12
...
- 02ac732: fix(cli): a redacted Cloudflare fault is named for what it is (#559)
...
- ed35536: Add `substrat scope domain <scopeId> --domain <fqdn>` — bind a custom domain to ANY owned scope,
...
- 64a1451: `substrat scope bind <scopeId> --version <id>` — the per-scope rollout primitive, reached
...
- 5a9d7bd: `substrat push` with neither `substrat.runtimeNeeds` nor a wrangler.jsonc now
...
- 7c9deca: `substrat versions <slug>` now agrees with `hostnames <slug>` on what a bare product
...
- 3565c4c: `substrat version` (and `--version` / `-v`) prints the CLI version, and an out-of-date
...
--version` (the _vertical's_ semver) — those remain flags to their commands; only the
  ...
  is the authority on whether this CLI is still _compatible_. Authenticated responses may
...
- 81f0022: `scope pull`/`scope restore` now emit FK-ordered dumps. A dump's `tables` array
