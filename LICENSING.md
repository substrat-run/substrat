...
| `examples/*`, `spikes/*` | **No licence field** — so no grant today | Not workspace members and not products: `examples/external-vertical` exists to prove an npm install works, and a spike is the evidence an RFC cites. Neither carries a `license`, so neither grants anything; whether `examples/*` *should* be Apache-2.0 like the demos is an open question, not a decision recorded here. |
