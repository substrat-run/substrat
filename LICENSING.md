...
| `@substrat-run/cli`, `create-substrat`, `@substrat-run/model-emit`, `@substrat-run/model-view`, `@substrat-run/boundary-lint` | **Apache-2.0** | The tools you run against *your own* code — the command line, the scaffold and its template, the model emitter, the renderer that turns an emitted model into a readable page, the layer rules. Same reason as `contracts`: a build tool that copyleft-captured its input would capture every vertical built with it. |
