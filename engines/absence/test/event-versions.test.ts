 ...
 *
 * `absenceEventVersions` is what `emitAbsenceEvent` stamps and what the manifest's
...
import { absenceEventVersions, emitAbsenceEvent, type AbsenceEventType } from '../src/events.js';
import { absenceManifest } from '../src/index.js';
...
describe('absence: event schemaVersion has one home', () => {
  it('the manifest emits exactly what shipped, in the same order', () => {
    expect(absenceManifest.events.emits).toEqual(SHIPPED);
  });
  ...
  it('every emission carries the version the manifest declares for its type', () => {
    ...
    for (const type of Object.keys(absenceEventVersions) as AbsenceEventType[]) {
      // The payload is irrelevant to the stamp; the cast is this test's, not the engine's.
      emitAbsenceEvent(ctx, { type, entity: { entityType: 'x', entityId: 'y' }, piiClass: 'none', payload: {} } as never);
    }
    expect(seen.map((e) => ({ type: e.type, schemaVersion: e.schemaVersion }))).toEqual(
      absenceManifest.events.emits.map((d) => ({ type: d.type, schemaVersion: d.schemaVersion })),
    );
  });
});
