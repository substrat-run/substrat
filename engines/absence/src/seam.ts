 ...
 *
 * **Parse always** — including the ledger fold. A balance is the sum of every
 * `delta` in the ledger, so a `delta` that drifted is the one value here that
 * would otherwise cross as a *number nobody questions*: wrong on a screen, never
 * a throw. Dev-only validation would be absent exactly where the version skew
 * lives — in production, against an engine nobody in this repo deployed. Every
 * read here is one row, one subject's ledger, one page (#811), or one date
 * window, so the parsed set is bounded by the caller's own window.
 */
import { substratError, validationIssuesFrom, z } from '@substrat-run/contracts';

/** How this engine names itself in a seam failure. */
const ENGINE = 'engine-absence';
...
/**
 * The SELECT list a schema describes: `'id, subject_type, subject_id, …'`.
 *
