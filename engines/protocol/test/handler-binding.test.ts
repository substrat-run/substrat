/**
 * #959 — engine-protocol's handlers are bound to its declared operations, as a
 * compile-time suite.
 *
 * `src/index.ts` writes its handler map `satisfies OperationHandlersFor<typeof
 * protocolOperations>`, so each handler's input and output are DERIVED from the declaration
 ...
 * `pnpm --filter @substrat-run/engine-protocol typecheck` goes red. Each negative has a
...
import { protocolModule, protocolOperations } from '../src/index.js';

type Ops = typeof protocolOperations;
...
export const pagedOk: Handlers['protocol/list-for-entity'] = async () => declared<Out<'protocol/list-for-entity'>>();
// @ts-expect-error 'protocol/list-for-entity' is paged: the handler returns a Page, not its entries
export const pagedBare: Handlers['protocol/list-for-entity'] = async () => declared<Out<'protocol/list-for-entity'>['entries']>();

// --- a read that declares one object returns one object, not a list of them ---
export const singleOk: Handlers['protocol/get'] = async () => declared<Out<'protocol/get'>>();
// @ts-expect-error 'protocol/get' declares one object, and a list of them is not it
export const singleList: Handlers['protocol/get'] = async () => declared<Out<'protocol/get'>[]>();

// --- a handler needs no more of its input than the host parses ----------------
export const inputOk: Handlers['protocol/get'] = async (_ctx, input: In<'protocol/get'>) => {
  void input;
  return declared<Out<'protocol/get'>>();
};
// @ts-expect-error 'undeclared' is not in the declared input, so the host never parsed it
export const inputUndeclared: Handlers['protocol/get'] = async (
  _ctx,
  input: In<'protocol/get'> & { undeclared: string },
) => {
  void input;
  return declared<Out<'protocol/get'>>();
};
