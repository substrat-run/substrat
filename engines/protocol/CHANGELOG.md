...
- 2b5d215: Every engine now publishes its event map, and its own emit sites are checked
  against it.
  ...
  **engine-absence turned out to have the same defect shape, unfiled.**
  ...
  emit _different_ event types — `absence.cancelled` and `absence.expired`. One
  ...
  **Types only, no runtime change.** `emit<Engine>Event` forwards to `ctx.emit`
  ...
  of it. A type-level constraint fails _permissively_, so a decorative constraint
...
- b81f21b: engine-protocol declares its state machine. The four states a protocol instance can hold — `open`, `pending_signature`, `signed`, `voided` — and every verb that moves it between them are now a `defineLifecycles` declaration the compiler checks against the entity's own `status` enum and the engine's declared operations, and the machine is emitted into `model.json` so widening it has to appear in a diff. `protocolLifecycles` and `protocolLifecycle` are exported, so an app can derive "what can I do from here" from the engine instead of re-deriving `status === 'open' && …` in a button.
