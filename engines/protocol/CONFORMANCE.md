...
# Conformance receipt — @substrat-run/engine-protocol

14 operations · 9 narrowed checks · 8 conformance pairs driven
...
**8 pairs driven** across 8 of this package's 9 narrowed checks.

| Operation | Permission | Narrows to | Driven |
| --- | --- | --- | --- |
| `protocol/bind-document` | `protocol:bind` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/cancel-signatures` | `protocol:request-signature` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/countersign` | `protocol:countersign` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/fill` | `protocol:fill` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/get` | `protocol:read` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/list-for-entity` | `protocol:read` | type from `entityType`, id from `entityId` | — see §3 — |
| `protocol/request-signatures` | `protocol:request-signature` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/sign` | `protocol:sign` | `protocol`, id from `instanceId` | `protocol` |
| `protocol/void` | `protocol:void` | `protocol`, id from `instanceId` | `protocol` |
...
| --- | --- |
| `protocol/list-for-entity` | declares 'entityFrom: entityType', whose schema does not enumerate the types it admits — the kit cannot know which entity to create, and will not guess one |
...
| --- | --- | --- |
| Node-level check | 5 | `protocol/decline-signature`, `protocol/define-template`, `protocol/instantiate`, `protocol/list-templates`, `protocol/record-signature` |
