 ...
 * `packages/kernel/src/scope-host.ts`); `emitBookingEvent` forwards to `ctx.emit`, adding only the
 * `schemaVersion` it stamps from `bookingEventVersions` (#1597).
 *
 ...
 *
 * Its one runtime act is stamping `schemaVersion` from `bookingEventVersions` onto
