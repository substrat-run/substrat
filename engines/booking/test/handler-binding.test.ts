/**
 * #959 — engine-booking's handlers are bound to its declared operations, as a
 * compile-time suite.
 *
 * `src/index.ts` writes its handler map `satisfies OperationHandlersFor<typeof
 * bookingOperations>`, so each handler's input and output are DERIVED from the declaration
 ...
 * `pnpm --filter @substrat-run/engine-booking typecheck` goes red. Each negative has a
...
import { bookingModule, bookingOperations } from '../src/index.js';

type Ops = typeof bookingOperations;
...
export const pagedOk: Handlers['booking/list'] = async () => declared<Out<'booking/list'>>();
// @ts-expect-error 'booking/list' is paged: the handler returns a Page, not its entries
export const pagedBare: Handlers['booking/list'] = async () => declared<Out<'booking/list'>['entries']>();

// --- a read that declares one object returns one object, not a list of them ---
export const singleOk: Handlers['booking/get'] = async () => declared<Out<'booking/get'>>();
// @ts-expect-error 'booking/get' declares one object, and a list of them is not it
export const singleList: Handlers['booking/get'] = async () => declared<Out<'booking/get'>[]>();

// --- a handler needs no more of its input than the host parses ----------------
export const inputOk: Handlers['booking/get'] = async (_ctx, input: In<'booking/get'>) => {
  void input;
  return declared<Out<'booking/get'>>();
};
// @ts-expect-error 'undeclared' is not in the declared input, so the host never parsed it
export const inputUndeclared: Handlers['booking/get'] = async (
  _ctx,
  input: In<'booking/get'> & { undeclared: string },
) => {
  void input;
  return declared<Out<'booking/get'>>();
};
