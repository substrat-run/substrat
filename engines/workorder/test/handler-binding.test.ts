/**
 * #959 — engine-workorder's handlers are bound to its declared operations, as a
 * compile-time suite.
 *
 * `src/index.ts` writes its handler map `satisfies OperationHandlersFor<typeof
 * workorderOperations>`, so each handler's input and output are DERIVED from the declaration
 ...
 * `pnpm --filter @substrat-run/engine-workorder typecheck` goes red. Each negative has a
...
import { workorderModule, workorderOperations } from '../src/index.js';

type Ops = typeof workorderOperations;
...
export const pagedOk: Handlers['workorder/list'] = async () => declared<Out<'workorder/list'>>();
// @ts-expect-error 'workorder/list' is paged: the handler returns a Page, not its items
export const pagedBare: Handlers['workorder/list'] = async () => declared<Out<'workorder/list'>['items']>();
...
export const inputOk: Handlers['workorder/get'] = async (_ctx, input: In<'workorder/get'>) => {
  void input;
  return declared<Out<'workorder/get'>>();
};
// @ts-expect-error 'undeclared' is not in the declared input, so the host never parsed it
export const inputUndeclared: Handlers['workorder/get'] = async (
  _ctx,
  input: In<'workorder/get'> & { undeclared: string },
) => {
  void input;
  return declared<Out<'workorder/get'>>();
};
