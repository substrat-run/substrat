...
- The endpoints a document names are held to one rule, decided against the configured issuer: `https` always, and plaintext only on a loopback host when the issuer is itself a loopback `http` issuer. An `https` issuer's document therefore cannot name a plaintext loopback endpoint. What happens on a breach depends on the endpoint. A document with a plaintext `jwks_uri` or `authorization_endpoint` is refused. A plaintext token endpoint fails the login before anything is sent. A plaintext UserInfo endpoint is skipped and the login continues without it. The id-token hint is withheld from a plaintext end-session endpoint. The token endpoint may sit on another origin than the issuer.
