...
The scaffold's worker flags `x-substrat-exported-events` on its responses. The router passes that flag on its drain kick, and the control plane runs the producer's edges when it sees it. The control plane runs the phase on its scheduled sweep, with `CROSS_VERTICAL_CONSUMERS_PER_PASS` as the per-pass cap (`0` pauses).
