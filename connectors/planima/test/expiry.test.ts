...
/**
 * #1690 — Planima is a static token whose provider reports no expiry (api.ts). A connection created the way the connect flows create one records none, and the fleet health row says "not reported" — true, not a gap.
 */
describe('planima connection expiry', () => {
  let dir: string;
  afterEach(() => rmSync(dir, { recursive: true, force: true }));

  async function connect(expiresAt?: string) {
    dir = mkdtempSync(join(tmpdir(), 'substrat-planima-expiry-'));
