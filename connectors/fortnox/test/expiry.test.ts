...
/**
 * #1690 — Fortnox mints 1h access tokens via client_credentials and has no refresh token (api.ts), so there is no refresh horizon; the access-token expiry it computes must never be stored here. A connection created the way the connect flows create one records none, and the fleet health row says "not reported" — true, not a gap.
 */
describe('fortnox connection expiry', () => {
  let dir: string;
  afterEach(() => rmSync(dir, { recursive: true, force: true }));

  async function connect(expiresAt?: string) {
    dir = mkdtempSync(join(tmpdir(), 'substrat-fortnox-expiry-'));
    ...
    await host.admin.createConnection(staff, {
      ...
      label: 'Fortnox',
      ...(expiresAt ? { expiresAt } : {}),
      secret: { clientId: 'ci', clientSecret: 'cs', tenantId: '123456' },
    });
