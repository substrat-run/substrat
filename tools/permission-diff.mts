...
interface ModuleLike {
  manifest: { id: string; permissions: PermissionDecl[]; schedules?: ScheduleLike[]; events?: EventsLike };
}
interface RoleLike {
  key: string;
  permissions: string[];
...
function render(rel: string, pkg: string, src: Surface, regenerate: string, entry: string): string {
  ...
  if (modules.length === 0 && roles.length === 0 && grants.length === 0) {
    return [
      `<!-- GENERATED by tools/permission-diff.mts from ${entry} — do not edit by hand.`,
...
for (const { rel, dir, entry, pkg } of verticals) {
  ...
  const artifacts: { path: string; content: string }[] = [
    { path: join(dir, 'PERMISSIONS.md'), content: render(rel, pkg, src, regenerateFor(dir), entry) },
  ];
